...
contract PolySignalOrigin {
    struct TradeSignalInput {
    ...
    uint64 public totalSignalsObserved;
    ...
    function reportTradeSignal(TradeSignalInput calldata tradeInput)
        external
        onlyRelayer
        returns (uint64 sequence)
    {
        if (tradeInput.trader == address(0)) revert InvalidTrader(tradeInput.trader);
        if (tradeInput.direction > 1) revert InvalidDirection(tradeInput.direction);
        if (bytes(tradeInput.marketTitle).length == 0) revert EmptyMarketTitle();
        ...
        sequence = ++totalSignalsObserved;

        emit TradeSignalObserved(
